...
:::{#def-graphe-oriente-formel}
# Définition formelle d'un graphe orienté
...
Latex(
    r"$A=\left\{"
    + r",\;".join(["(" + str(e[0]) + "," + str(e[1]) + ")" for e in G.edges])
    + r"\right\}$"
)
```

:::{#def-degre-entrant-sortant}
# Degré entrant et sortant d'un sommet
...
Latex(
    "$"
    + r",\;".join(["d_-(" + str(s) + ")=" + str(d) for (s, d) in G.in_degree()])
    + "$"
)
```
...
Latex(
    "$"
    + r",\;".join(["d_+(" + str(s) + ")=" + str(d) for (s, d) in G.out_degree()])
    + "$"
)
```
